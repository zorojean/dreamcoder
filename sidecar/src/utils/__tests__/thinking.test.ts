...
describe('provider-aware thinking support', () => {
  ...
  beforeEach(() => {
    ...
    originalExplicitDisabledThinking = process.env.DREAMCODER_SEND_DISABLED_THINKING
  ...
  afterEach(() => {
    ...
    restoreEnv('DREAMCODER_SEND_DISABLED_THINKING', originalExplicitDisabledThinking)
  ...
  test('only sends explicit disabled thinking when the provider opts in', () => {
    delete process.env.DREAMCODER_SEND_DISABLED_THINKING
    expect(shouldSendExplicitDisabledThinking()).toBe(false)

    process.env.DREAMCODER_SEND_DISABLED_THINKING = '1'
