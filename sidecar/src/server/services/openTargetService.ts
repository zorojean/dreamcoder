...
async function defaultConvertIconToPng(iconPath: string, size: number): Promise<Uint8Array> {
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), 'dreamcoder-open-target-icon-'))
...
$source = $env:DREAMCODER_ICON_SOURCE
$output = $env:DREAMCODER_ICON_OUTPUT
...
`
  ...
  await execFile('powershell.exe', [
    ...
    env: {
      ...process.env,
      DREAMCODER_ICON_SOURCE: iconPath,
      DREAMCODER_ICON_OUTPUT: outputPath,
    },
    timeout: 5_000,
    windowsHide: true,
  })
