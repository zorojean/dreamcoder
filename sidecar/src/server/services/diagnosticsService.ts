...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'dreamcoder', 'diagnostics')
  }
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `dreamcoder-diagnostics-${timestamp}.tar.gz`
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'dreamcoder', 'providers.json')
    try {
